...
(function(){
	...
	window.isIE=navigator.appVersion.match(/MSIE/);
	if (window.isIE) {
	...
	Me.goPage=function(){
    	...
    	if (key!==null){
			try{
				...
				Me.prepState[key]=0;
				window.setTimeout(Me.ajaxPrepSubmit,1000);
				Me.init();
...
ECSideUtil.showShadowRow=function(formid,crow,eventSrc){
		var ecsideObj=ECSideList[formid];
...
ECSideUtil.ajaxRequest=function(url,callback,pars,asy){
	if (!asy){
		asy=true;
	}
	if (!pars){
	...
	var myAjax = new Ajax.Request( url,{requestHeaders:ECSideConstants.AJAX_HEADER, method: "POST", 
		asynchronous: asy , parameters: pars, onComplete: callback } );
	return myAjax;
};
