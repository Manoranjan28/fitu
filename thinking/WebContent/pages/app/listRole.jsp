...
<!-- Search List Start -->
<div id="result">
<ec:table items="list" var="role" retrieveRowsCallback="limit" 
	sortRowsCallback="limit" autoIncludeParameters="false" action="${ctxPath}/app/listRole.action">
    <ec:exportXls view="xls" fileName="Roles.xls" tooltip="Export Excel"/>
    <ec:row>
        <ec:column property="name"  title="RoleName" />
	    ...
	    <ec:column property="checkbox" title=" " sortable="false" viewsAllowed="html" style="width: 20px;">
	        <input type="checkbox" name="items" value="<c:out value="${role.id}" />" style="border:0px"/>
	    </ec:column>
    </ec:row>
</ec:table>
</div>
